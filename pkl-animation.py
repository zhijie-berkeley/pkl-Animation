...
def pklAnimation(time, dist, name, save):
    ...
    # save animation
    if save:
        anim.save('animation_pkl.mp4', writer="ffmpeg", fps=50, metadata={'title':'Animation of ploting ' + name +'.pkl'})
    else:
        plt.show()
...
def pklNpsAnimation(time, dist, name, save):
    ...
    for pos in npss:
        ax.axhline(pos, ls="--", color="k")
    ax.text(580, 880, 'NPS Entry')
    ax.text(580, 952, 'dyad')
    ax.text(580, 1026, 'NPS Exit')

    def init():
        line.set_data([], [])
        time_text.set_text('Start')
        # pos_text.set_text('NPS Entry')
        return line, time_text
    ...
    # save animation
    if save:
        anim.save('animation_pkl_nps.mp4', writer="ffmpeg", fps=50, metadata={'title':'Animation of ploting ' + name +'.pkl'})
    else:
        plt.show()
